...
import { RefreshCw, PanelRightClose, PanelRightOpen, Maximize2 } from "lucide-react";
...
export default function KnowledgeHub({ mode, width, onToggle }: KnowledgeHubProps) {
    const isCollapsed = mode === "collapsed";

    return (
        <aside
            className="shrink-0 border-l border-white/5 bg-[#0c1527] overflow-hidden transition-all duration-300 ease-in-out relative hidden lg:flex flex-col"
            style={{ width }}
        >
            {/* Collapsed state — just a toggle button strip */}
            {isCollapsed && (
                <button
                    onClick={onToggle}
                ...
                >
                    <PanelRightOpen className="w-5 h-5 text-slate-500 hover:text-slate-300 transition-colors" />
                </button>
            )}
                    ...
                    <div className="px-5 pt-5 pb-4 border-b border-white/5 shrink-0">
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-1">
                                ...
                                <button
                                    onClick={onToggle}
                                    className="ml-1 p-1 text-slate-500 hover:text-slate-300 rounded hover:bg-white/5 transition-colors"
                                    title={mode === "expanded" ? "Minimize" : "Expand"}
                                >
                                    {mode === "expanded" ? (
